package org.genesez.platforms.common.statistic;
...
public class StatisticObserver extends FileTreeObserverAdapter {
	...
	public void setFileTreeWalkerProperties(Properties properties) {
		...
		if (excludes.isEmpty()) {
			excludes.addAll(WorkflowUtils.split(this.properties.getProperty("excludes")));
		}
		if (generated.isEmpty()) {
			generated.addAll(WorkflowUtils.split(this.properties.getProperty("genDirs")));
		}
		for (String s : manual) {
	...
	public void setGenerates(String dirs) {
		...
		if (dirs.length() > 0) {
			List<String> list = WorkflowUtils.split(dirs);
			for (String s : list) {
				setGenerate(s);
			}
		}
