...
public class Mapping {
	...
	public static String getMapTo(String _map, String _mapTo) {
		try {
			...
			return getMapToValue(doc, _map, _mapTo);			
		} catch (Exception e) {
	...
	private static String getMapToValue(Document _doc, String _map, String _mapTo) {
		Element mapping, mapTo;
		List mappings;
		...
		while(it.hasNext()) {
			...
			if (isSearchedMap(getMap(mapping), _map)) {
				mapTo = getMapTo(getToDefault(mapping), getToOther(mapping, _mapTo));
				return getValue(mapTo);
			}
		}
		return _map;
	}
	...
	private static Element getMapTo(Element toDefault, Element toOther) {
		if (toOther != null) {
			return toOther;
		} else {
			return toDefault;
	...
	private static Element getToDefault(Element mapping) {
		return mapping.element("ToDefault");
	}
	
	
	private static Element getToOther(Element mapping, String mapTo) {
