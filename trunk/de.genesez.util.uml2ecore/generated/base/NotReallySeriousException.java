package base;


/*
        NotReallySeriousException.java
        generated @ 2007-05-24 19:39:01.781 CEST by georg
        here only for demo of java extension call.
        should not be used because it would cause
        new svn revisions even when nothing else changed!
 */

/* PROTECTED REGION ID(java.imports._OCniwAodEdy9NY2aWONLeQ) ENABLED START */
// put your imports here - following import is placeholder so that your IDE puts further imports here
import java.lang.String;


/* PROTECTED REGION END */

/**
        <!-- PROTECTED REGION ID(java.type.comment._OCniwAodEdy9NY2aWONLeQ) ENABLED START -->
        <!-- TODO put your type comment here [description + responsibility] -->
        <!-- PROTECTED REGION END -->

        @author                georg
        @version        2007-05-24 19:39:01.781 CEST
 */
public class NotReallySeriousException extends Exception {
    // -- constructors ----------------------------------------------
    // -- abstract methods ------------------------------------------
    // -- methods ---------------------------------------------------

    // -- attribute definitions with default initialization ---------

    // -- attribute getter + setter ---------------------------------

    /* PROTECTED REGION ID(java.impl._OCniwAodEdy9NY2aWONLeQ) ENABLED START */
    // -- put your implementation code here -------------------------

    /* PROTECTED REGION END */
}
