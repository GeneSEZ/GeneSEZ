...
/**
 * Ausführbare(s) Template, Skript oder Regel, die in einem Skript enthalten ist. Die parse-Methode analysiert, welche Executables verwendet werden. Wird in Subklassen ausprogrammiert.
 * @author EmpeROOR
 */
...
//		metaType = m;
	...
	public String getQname() {
		...
		throw new UnsupportedOperationException("The implementation of the derived attribute 'QNAME' is missing!");
		/* PROTECTED REGION END */
	}
	...
	protected final String OUTCOMMENTEDEXTFUNCTION = "(?s)/\\*\\*?(.*?)\\*/";//"(?m)^\\s*/?\\*\\s*(.*)"; //"(?s)/\\*\\*?(.*?)\\*/"; (?s)/\*\*?(.*?)\*/
	...
	public String toString() {
		...
		if (name != null && metaType != null && script != null)
			return name + "(" + paramDef + ") MetaType: " + metaType + " @ " + script.getFile().getName();
		else
			return "";
	}
			...
			if(this.getMetaType() != null) {
				if(toCompare.getMetaType() != null) {
					if(!this.getMetaType().equals(toCompare.getMetaType())) {
						return this.getMetaType().compareTo(toCompare.getMetaType());
					}
				}
			}
		}		
	...
	protected boolean callerExists(Executable caller) {
		for (Executable e : this.getCaller()) {
			if(e.getName().equals(caller.getName())
					&& e.getScript().getFile().getName().equals(caller.getScript().getFile().getName())
		...
		for (Executable e : this.getCalled()) {
			if(e instanceof Template){
				if(e.getName().equals(searched.getName())
						&& searched.getScript().getFile().equals(e.getScript().getFile())) {
					return true;
				}
			}else if(e instanceof Function){
				// the file isnt setted here.. we set it up later
				if(e.getName().equals(searched.getName()) && e.getParamDef().equals(searched.getParamDef())) {
					return true;
				}
			}
		}
	...
	protected boolean buildFunction(char[] chars) {
		...
		// if any char of pCrap is located, the function name will be resetted to empty string
		Pattern pCrap = Pattern.compile("!|\\.|&|:|\\)|\\?|/|\\*|\\||=|\r\n|\r|\n|\\s");
		Matcher mCrap;
		for (int j = 0; j < chars.length - 1; j++) {
			...
			if(StringHelper.isWordChar(chars[j]) && chars[j + 1] == '(') {
				...
				// TODO 
				params = checkBracketCount(checkBracketCount(findParams(j + 1, chars)));
