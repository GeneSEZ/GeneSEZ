...
public class RelativePath {
	...
	private static List<String> getPathList(File f) {
		...
		try {
			...
			System.err.println("erroneous file name " + f.getAbsolutePath());
			e.printStackTrace();
			l = null;
		}
		return l;
	 ...
	 * @param home
	 *            home path
	 * @param pathToSearchedFile
	 *            path of file
	 */
	private static String matchPathLists(List<String> home,
			List<String> pathToSearchedFile) {
		int i, j;
		String s = "";
		...
		while ((i >= 0) && (j >= 0)
				&& (home.get(i).equals(pathToSearchedFile.get(j)))) {
			i--;
			j--;
		}
		for (; i >= 0; i--) {
